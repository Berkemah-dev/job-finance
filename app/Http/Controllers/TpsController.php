...
class TpsController extends Controller
{
    ...
    {
        $tps = Tps::create($this->validated($request));
    ...
    public function update(Request $request, Tps $tps, MasterDataService $service)
    {
        ...
        $tps->update($this->validated($request, $tps));
    ...
    private function validated(Request $request, ?Tps $tps = null): array
    {
        ...
        $data = $request->validate([
            ...
            'code' => ['required', 'string', 'max:30', 'unique:tps,code'.($tps ? ','.$tps->id : '')],
        ...
        if ($request->exists('is_active')) {
            ...
            $data['is_active'] = $tps ? $tps->is_active : true;
        }
