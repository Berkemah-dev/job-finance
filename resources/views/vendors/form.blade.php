...
@endforeach
<div class="field"><label for="type">Kategori utama <span class="required">*</span></label><select id="type" name="type" required>@foreach(config('operations.vendor_types') as $value=>$label)<option value="{{ $value }}" @selected(old('type',$vendor->type)===$value)>{{ $label }}</option>@endforeach</select></div>
...
<div class="field span-2"><label>Kategori tambahan</label><div class="check-grid">@php($checked = collect(old('categories',$vendor->categories?->pluck('category')->all() ?? [])))@foreach(config('operations.vendor_types') as $value=>$label)<label class="check-item"><input type="checkbox" name="categories[]" value="{{ $value }}" @checked($checked->contains($value))> {{ $label }}</label>@endforeach<small class="form-help">Opsional; menandai vendor untuk lebih dari satu kategori (memengaruhi filter daftar vendor).</small></div></div>
