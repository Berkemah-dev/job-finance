...
/>
<section class="panel"><div class="panel-heading"><h2>Pengguna terdaftar</h2><span class="count-badge">Direktori pengguna</span></div><div class="table-scroll"><table><thead><tr><th>Nama</th><th>Email</th><th>Role</th><th>Bergabung</th><th>Aksi</th></tr></thead><tbody>@foreach($users as $user)<tr><td><strong>{{ $user->name }}</strong></td><td>{{ $user->email }}</td><td><span class="role-badge">{{ $user->role?->label ?? 'Tanpa role' }}</span></td><td>{{ $user->created_at->format('d/m/Y') }}</td><td>@can('users.manage')<div class="table-actions"><a class="btn-action" href="{{ route('users.edit',$user) }}">Edit</a></div>@endcan</td></tr>@endforeach</tbody></table></div><div class="pagination">{{ $users->links() }}</div></section>
