...
@if($role === 'operational' && isset($unfinishedJobs))
<section class="panel" style="margin-bottom:20px"><div class="panel-heading"><div><h2>Job belum final</h2><p>Job yang masih memiliki biaya Draft.</p></div><span class="count-badge">{{ $unfinishedJobs->count() }}</span></div><div class="table-scroll"><table><thead><tr><th>Job</th><th>Subject</th><th>Status biaya</th></tr></thead><tbody>@forelse($unfinishedJobs as $job)<tr><td><a class="text-link" href="{{ route('jobs.show',$job) }}">{{ $job->number }}</a></td><td>{{ $job->subject }}</td><td>Belum final</td></tr>@empty<tr><td colspan="3">Semua biaya job sudah final.</td></tr>@endforelse</tbody></table></div></section>
@endif

@if($role === 'customer-service' && isset($arrivalSoon))
<section class="panel" style="margin-bottom:20px"><div class="panel-heading"><div><h2>Job mendekati tiba</h2><p>ETA dalam 14 hari ke depan.</p></div><span class="count-badge">{{ $arrivalSoon->count() }}</span></div><div class="table-scroll"><table><thead><tr><th>Job</th><th>Subject</th><th>ETA</th></tr></thead><tbody>@forelse($arrivalSoon as $job)<tr><td><a class="text-link" href="{{ route('jobs.show',$job) }}">{{ $job->number }}</a></td><td>{{ $job->subject }}</td><td>{{ $job->eta ? $job->eta->format('d/m/Y') : '—' }}</td></tr>@empty<tr><td colspan="3">Tidak ada job yang mendekati tiba.</td></tr>@endforelse</tbody></table></div></section>
@endif

@if(isset($widgets['quotes']))
<div class="section-heading"><h2>Pipeline quotation</h2><span class="subtle">{{ $widgets['quotes30d'] ?? 0 }} penawaran dalam 30 hari terakhir</span></div>
<div class="stats-grid">
