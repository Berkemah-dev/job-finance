...
@endforeach
@can('reports.view')
@foreach([['Piutang Customer','wallet','amber','Tagihan yang belum dibayar'],['Profit Job','chart','purple','Nilai jual dikurangi modal provision']] as [$label,$icon,$color,$caption])
<article class="stat-card"><div class="stat-top"><span>{{ $label }}</span><span class="stat-icon {{ $color }}"><x-icon :name="$icon"/></span></div><strong class="stat-number"><small>Rp</small> 0</strong><p>{{ $caption }}</p></article>
@endforeach
@endcan
</div>
@can('reports.view')
<div class="finance-strip">@foreach(['Temporary','Modal Provision','Pendapatan','HPP'] as $label)<div><span>{{ $label }}</span><strong>Rp 0</strong></div>@endforeach</div>
@endcan
...
@can('reports.view')
<div class="dashboard-grid bottom-grid"><section class="panel"><div class="panel-heading"><div><h2>Pendapatan & profit</h2><p>Ringkasan performa bulanan.</p></div><span class="count-badge">{{ date('Y') }}</span></div><div class="empty-state compact"><span class="empty-icon"><x-icon name="chart"/></span><h3>Belum ada data keuangan</h3><p>Grafik tersedia setelah ada job yang ditutup.</p></div></section><section class="panel"><div class="panel-heading"><div><h2>Invoice belum lunas</h2><p>Tagihan yang perlu ditindaklanjuti.</p></div><span class="count-badge">0</span></div><div class="empty-state compact"><span class="empty-icon"><x-icon name="file"/></span><h3>Belum ada invoice</h3><p>Invoice akan dibuat saat closing job.</p></div></section></div>
@endcan
<p class="demo-note">Nilai penawaran merupakan estimasi. Ringkasan keuangan akan terisi dari biaya aktual, closing, dan pembayaran pada tahap berikutnya.</p>
@endsection
