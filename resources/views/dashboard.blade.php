...
@can('reports.view')
<div class="dashboard-grid bottom-grid"><section class="panel"><div class="panel-heading"><div><h2>Pendapatan & profit</h2><p>Akumulasi job yang sudah ditutup.</p></div><span class="count-badge">{{ date('Y') }}</span></div><div class="summary-box"><div class="summary-row"><span>Pendapatan</span><strong>Rp {{ \App\Support\Money::format($revenueBalance) }}</strong></div><div class="summary-row"><span>HPP</span><strong>Rp {{ \App\Support\Money::format($cogsBalance) }}</strong></div><div class="summary-row summary-total"><span>Profit</span><strong>Rp {{ \App\Support\Money::format($profitBalance) }}</strong></div></div></section><section class="panel"><div class="panel-heading"><div><h2>Invoice belum lunas</h2><p>Tagihan yang perlu ditindaklanjuti.</p></div><span class="count-badge">{{ $unpaidInvoices->count() }}</span></div><div class="table-scroll"><table><thead><tr><th>Invoice</th><th>Jatuh tempo</th><th class="money">Sisa</th></tr></thead><tbody>@forelse($unpaidInvoices as $invoice)<tr><td><a class="text-link" href="{{ route('invoices.show',$invoice) }}">{{ $invoice->number }}</a></td><td>{{ $invoice->due_date->format('d/m/Y') }}</td><td class="money">Rp {{ \App\Support\Money::format($invoice->balance) }}</td></tr>@empty<tr><td colspan="3">Belum ada invoice terbuka.</td></tr>@endforelse</tbody></table></div></section></div>
@endcan
