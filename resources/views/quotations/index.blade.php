...
<div class="table-scroll"><table><thead><tr><th>Nomor / Tanggal</th><th>Customer / Penawaran</th><th>Layanan</th><th>Asal → Tujuan</th><th>Sales</th><th>Mata uang</th><th class="money">Total sebelum pajak</th><th>Status</th><th>Aksi</th></tr></thead><tbody>@forelse($quotations as $quotation)<tr><td><strong>{{ $quotation->number }}</strong><br><small>{{ $quotation->quotation_date->format('d/m/Y') }}</small></td><td>{{ $quotation->customer_snapshot['name'] }}<br><small>{{ Str::limit($quotation->subject,45) }}</small></td><td>{{ \App\Models\ServiceType::label($quotation->service_type) }}</td><td>@if($quotation->origin || $quotation->destination){{ $quotation->origin ?? '—' }} → {{ $quotation->destination ?? '—' }}@else—@endif</td><td>{{ $quotation->sales?->name ?? $quotation->creator?->name ?? '—' }}</td><td>{{ $quotation->currency }}</td><td class="money">Rp {{ \App\Support\Money::format($quotation->subtotal) }}</td><td><span class="status-badge status-{{ $quotation->status->value }}">{{ $quotation->status->label() }}</span>@if($quotation->status === \App\Enums\QuotationStatus::Approved && !$quotation->job)<br><small class="subtle">Siap dibuat Job</small>@endif</td><td><div class="table-actions"><a class="btn-action btn-action-primary" href="{{ route('quotations.show',$quotation) }}" title="Detail Quotation" data-tooltip="Detail" aria-label="Detail Quotation"><x-icon name="eye"/></a>@can('convert',$quotation)@if(!$quotation->job)<form method="POST" action="{{ route('quotations.convert',$quotation) }}" data-confirm="Buat Job Order Draft dari quotation ini?">@csrf<input type="hidden" name="lock_version" value="{{ $quotation->lock_version }}"><button class="btn-action btn-action-success" title="Buat Job" data-tooltip="Buat Job" aria-label="Buat Job"><x-icon name="arrow"/></button></form>@endif@endcan</div></td></tr>@empty<tr><td colspan="9"><div class="empty-state"><x-icon name="file"/><h3>Belum ada quotation yang sesuai</h3><p>Buat penawaran pertama untuk customer Anda.</p></div></td></tr>@endforelse</tbody></table></div><div class="pagination">{{ $quotations->links() }}</div></section>
@endsection
