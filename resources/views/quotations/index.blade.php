            ...
            <tbody>
                @forelse($quotations as $quotation)
                    <tr>
                        ...
                        <td>{{ $quotation->customer_snapshot['name'] }}<br><small>{{ Str::limit($quotation->subject,45) }}</small></td>
                        ...
                        <td>
                            <div class="table-actions">
                                <a class="btn-action btn-action-primary" href="{{ route('quotations.show',$quotation) }}" title="Detail Quotation" data-tooltip="Detail" aria-label="Detail Quotation"><x-icon name="eye"/></a>
                                @can('convert', $quotation)
                                    @if(! $quotation->job)
                                        <form method="POST" action="{{ route('quotations.convert',$quotation) }}" data-confirm="Buat Job Order Draft dari quotation ini?">
                                            ...
                                            <button class="btn-action btn-action-success" title="Buat Job" data-tooltip="Buat Job" aria-label="Buat Job"><x-icon name="arrow"/></button>
                                        </form>
                                    @endif
                                @endcan
                            </div>
                ...
                @empty
                    <tr><td colspan="9"><div class="empty-state"><x-icon name="file"/><h3>Belum ada quotation yang sesuai</h3><p>Buat penawaran pertama untuk customer Anda.</p></div></td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
