...
<div class="table-scroll"><table><thead><tr><th>Pekan</th><th>Tanggal Berlaku</th><th>Mata Uang</th><th>Kurs</th><th>Service</th><th>Status</th><th>Aksi</th></tr></thead><tbody>@forelse($items as $item)<tr><td><strong>{{ $item->week }}</strong></td><td>{{ $item->effective_date?->format('d M Y') }}@if($item->effective_until)<br><small>s.d. {{ $item->effective_until->format('d M Y') }}</small>@endif</td><td>{{ $item->currency }}</td><td>{{ number_format((float) $item->exchange_rate, 2, ',', '.') }}</td><td>{{ config('operations.service_types.'.$item->service, $item->service ?? 'Semua') }}</td><td>@if($item->is_active)<span class="status-badge status-active">Aktif</span>@else<span class="status-badge status-inactive">Nonaktif</span>@endif</td><td>@can('pricing.manage')<div class="table-actions"><a class="btn-action" href="{{ route('pricing.weekly.edit',$item) }}">Edit</a><form method="POST" action="{{ route('pricing.weekly.toggle',$item) }}" data-confirm="{{ $item->is_active?'Nonaktifkan':'Aktifkan' }} weekly pricing ini?">@csrf<input type="hidden" name="lock_version" value="{{ $item->lock_version }}"><button class="btn-action {{ $item->is_active ? '' : 'btn-action-success' }}">{{ $item->is_active?'Nonaktifkan':'Aktifkan' }}</button></form><form method="POST" action="{{ route('pricing.weekly.destroy',$item) }}" data-confirm="Hapus weekly pricing ini?">@csrf @method('DELETE')<input type="hidden" name="lock_version" value="{{ $item->lock_version }}"><button class="btn-action btn-action-danger">Hapus</button></form></div>@endcan</td></tr>@empty<tr><td colspan="7"><div class="empty-state"><x-icon name="clock"/><h3>Belum ada weekly pricing</h3><p>Tambahkan weekly pricing atau sesuaikan filter.</p></div></td></tr>@endforelse</tbody></table></div><div class="pagination">{{ $items->links() }}</div></section>
@endsection