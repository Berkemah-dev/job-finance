...
<div class="field"><label for="effective_until">Berlaku sampai</label><input id="effective_until" type="date" name="effective_until" value="{{ old('effective_until',$weeklyPricing->effective_until?->format('Y-m-d')) }}"><small class="form-help">Kosongkan bila kurs berlaku tanpa batas.</small></div>
...
<div class="field"><label for="service">Service</label><select id="service" name="service"><option value="">Semua service</option>@foreach($serviceTypes ?? \App\Models\ServiceType::options() as $value=>$label)<option value="{{ $value }}" @selected(old('service',$weeklyPricing->service)===$value)>{{ $label }}</option>@endforeach</select></div>
