...
<div class="field"><label for="service">Service</label><select id="service" name="service"><option value="">Semua service</option>@foreach(config('operations.canonical_service_types') as $value=>$label)<option value="{{ $value }}" @selected(old('service',$weeklyPricing->service)===$value)>{{ $label }}</option>@endforeach</select></div>
