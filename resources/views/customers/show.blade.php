...
<div class="table-scroll"><table><thead><tr><th>Jenis</th><th>Nama file</th><th>Ukuran</th><th>Tipe</th><th>Diunggah oleh</th><th>Diunggah pada</th><th>Aksi</th></tr></thead><tbody>@foreach($customer->documents as $document)<tr><td><span class="status-badge status-approved">{{ \Illuminate\Support\Str::upper($document->type) }}</span></td><td>{{ $document->original_name ?? $document->filename }}</td><td>@if($document->size){{ number_format($document->size / 1024, 0, ',', '.') }} KB@else—@endif</td><td>{{ $document->mime ?? '—' }}</td><td>{{ $document->uploader?->name ?? '—' }}</td><td>{{ $document->created_at?->format('d/m/Y H:i') }}</td><td><div class="action-group"><a class="text-link" href="{{ route('customers.documents.download', [$customer, $document]) }}">Unduh</a>@can('customers.manage')<form method="POST" action="{{ route('customers.documents.destroy', [$customer, $document]) }}" data-confirm="Hapus dokumen ini?">@csrf @method('DELETE')<button class="text-link danger">Hapus</button></form>@endcan</div></td></tr>@endforeach</tbody></table></div>
@else
