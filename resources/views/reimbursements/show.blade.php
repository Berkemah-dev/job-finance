...
<div class="page-heading"><div><p class="eyebrow">REIMBURSEMENT</p><h1>{{ $reimbursement->number }} <span class="status-badge status-{{ $reimbursement->status }}">{{ $statusLabels[$reimbursement->status] ?? $reimbursement->status }}</span></h1><p>{{ $reimbursement->description }}</p></div><a class="button button-secondary" href="{{ route('reimbursements.index') }}">← Kembali</a></div>
