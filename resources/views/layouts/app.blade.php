        ...
        $checkItemActive = function ($item) use ($isCostPage) {
            ...
            if (str_starts_with((string)$destination, 'reports.')) {
                ...
                if ($destination === 'jobs.index') {
                    return (request()->routeIs('jobs.*', 'shipping-instructions.*', 'booking-confirmations.*', 'awbs.*', 'bills-of-lading.*')) && !$isCostPage;
                }
                ...
                @can($permission)
                    ...
                    @elseif($label === 'Data Document')
                        <details class="nav-subgroup" {{ request()->routeIs('document-types.*') ? 'open' : '' }}>
                            <summary class="nav-item {{ $active ? 'active' : '' }}" title="{{ $label }}"><x-icon :name="$icon"/><span>{{ $label }}</span><x-icon name="chevron-down" class="nav-subgroup-chevron"/></summary>
...
<div class="workspace">
    ...
    <main id="main" tabindex="-1">
        <div class="app-toast-wrap" aria-live="polite" aria-atomic="true">
            @if(session('success'))
                <div class="app-toast success" role="status" data-toast><span class="app-toast-icon">✓</span><div><strong>Berhasil</strong><p>{{ session('success') }}</p></div><button class="app-toast-close" type="button" aria-label="Tutup">×</button></div>
            @endif
            @if(session('error'))
            ...
            @endif
            @if($errors->any())
                <div class="app-toast error" role="alert" data-toast><span class="app-toast-icon">!</span><div><strong>Data belum lengkap</strong><ul>@foreach($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul></div><button class="app-toast-close" type="button" aria-label="Tutup">×</button></div>
            @endif
        </div>
        @yield('content')
    </main>
...
<div class="confirm-backdrop" data-confirm-modal aria-hidden="true">
    <div class="confirm-card" role="dialog" aria-modal="true" aria-labelledby="confirm-title">
        ...
        <p data-confirm-message>Apakah data ini ingin diproses?</p>
        <div class="confirm-actions">
            <button type="button" class="button button-secondary" data-confirm-cancel>Batal</button>
            <button type="button" class="button button-primary" data-confirm-ok>Lanjut</button>
        </div>
    </div>
</div>
</body>
