...
<head>
    ...
    <style>
        .theme-toggle{border:1px solid var(--border-color,#dbe3ef);background:var(--surface,#fff);color:var(--text,#0f1f3d);border-radius:10px;padding:8px 10px;cursor:pointer;display:inline-flex;align-items:center;gap:6px;font-size:12px}.port-autocomplete-field{border:1px solid #1e3a8a!important;box-shadow:inset 3px 0 0 #dc2626}.port-autocomplete-field:focus{border-color:#dc2626!important;box-shadow:0 0 0 3px rgba(220,38,38,.12),inset 3px 0 0 #1e3a8a!important}
...
<div class="workspace">
    <header class="topbar">
        ...
        <div class="topbar-right">
            <button type="button" class="theme-toggle" data-theme-toggle aria-label="Ganti tema" title="Ganti tema"><span data-theme-icon>☾</span><span data-theme-label>Gelap</span></button>
...
<script>
document.addEventListener('DOMContentLoaded',function(){const root=document.documentElement,button=document.querySelector('[data-theme-toggle]'),icon=document.querySelector('[data-theme-icon]'),label=document.querySelector('[data-theme-label]');if(!button)return;const sync=()=>{const dark=root.classList.contains('theme-dark');icon.textContent=dark?'☀':'☾';label.textContent=dark?'Putih':'Gelap';};button.addEventListener('click',()=>{root.classList.toggle('theme-dark');localStorage.setItem('jobfinance_theme',root.classList.contains('theme-dark')?'dark':'light');sync();});sync();});
</script>
</body>
</html>
