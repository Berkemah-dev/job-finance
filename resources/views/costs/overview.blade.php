...
<div class="table-scroll"><table><thead><tr><th>Job / Customer</th><th>Pekerjaan</th><th>Status job</th><th>Biaya Draft</th><th>Biaya Final</th><th>Aksi</th></tr></thead><tbody>@forelse($jobs as $job)<tr><td><strong>{{ $job->number }}</strong><br><small>{{ $job->quotation_snapshot['customer']['name'] }}</small></td><td>{{ Str::limit($job->subject,40) }}</td><td><span class="status-badge status-{{ $job->status }}">{{ config('operations.job_statuses.'.$job->status) }}</span></td><td>{{ $job->draft_costs_count }}</td><td>{{ $job->final_costs_count }}</td><td><div class="table-actions"><a class="btn-action btn-action-primary" href="{{ route('jobs.costs.index',$job) }}">Lihat Biaya</a></div></td></tr>@empty<tr><td colspan="6"><div class="empty-state"><x-icon name="wallet"/><h3>Belum ada job yang sesuai</h3><p>Job tersedia setelah quotation dikonversi.</p></div></td></tr>@endforelse</tbody></table></div><div class="pagination">{{ $jobs->links() }}</div></section>
@endsection
