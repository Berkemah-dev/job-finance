...
<div><dt>Jenis biaya</dt><dd><strong>{{ $cost->type === 'temporary' ? 'Reimburse' : 'Non Reimburse' }}</strong>@if($cost->cost_category)<br><span class="subtle" style="font-size: 12px;">{{ match($cost->cost_category) { 'reimbursement' => 'Reimbursement (Temporary Payment)', 'debit_note' => 'Debit Note (Piutang Agent)', 'credit_note' => 'Credit Note (Hutang Agent)', default => 'Payment Request (Piutang Customer)' } }}</span>@endif</dd></div><div><dt>Tanggal biaya</dt><dd>{{ $cost->cost_date->format('d/m/Y') }}</dd></div><div><dt>Jumlah / Satuan</dt><dd>{{ \App\Support\Money::format($cost->quantity) }} {{ $cost->unit }}</dd></div>
