...
<div class="field"><label for="container_type">Jenis kontainer</label><select id="container_type" name="container_type"><option value="">Pilih kontainer</option>@foreach($containerUnits ?? \App\Models\ContainerUnit::options() as $value=>$label)<option value="{{ $value }}" @selected(old('container_type',$job->container_type)===$value)>{{ $label }}</option>@endforeach</select></div>
