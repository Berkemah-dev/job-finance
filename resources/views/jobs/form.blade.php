...
<div class="form-section-heading"><h2>Rute & transportasi</h2><p>Informasi perjalanan dan sarana pengangkut.</p></div>
<div class="form-grid">
<div class="field"><label for="pol">Pelabuhan muat (POL)</label><input id="pol" name="pol" maxlength="120" value="{{ old('pol',$job->pol) }}" placeholder="Pelabuhan / kota keberangkatan"></div>
<div class="field"><label for="pod">Pelabuhan bongkar (POD)</label><input id="pod" name="pod" maxlength="120" value="{{ old('pod',$job->pod) }}" placeholder="Pelabuhan / kota tujuan"></div>
<div class="field"><label for="etd">Perkiraan berangkat (ETD)</label><input id="etd" name="etd" type="date" value="{{ old('etd',$job->etd?->format('Y-m-d')) }}"></div>
<div class="field"><label for="eta">Perkiraan tiba (ETA)</label><input id="eta" name="eta" type="date" value="{{ old('eta',$job->eta?->format('Y-m-d')) }}"></div>
<div class="field"><label for="vessel_voyage">Vessel & voyage</label><input id="vessel_voyage" name="vessel_voyage" maxlength="120" value="{{ old('vessel_voyage',$job->vessel_voyage) }}" placeholder="Nama kapal & voyage"></div>
...
<div class="form-section-heading"><h2>Referensi dokumen</h2><p>Nomor dokumen pengiriman yang diterbitkan atas muatan ini.</p></div>
<div class="form-grid">
<div class="field"><label for="bl_number">Nomor BL</label><input id="bl_number" name="bl_number" maxlength="60" value="{{ old('bl_number',$job->bl_number) }}" placeholder="Master Bill of Lading"></div>
<div class="field"><label for="hbl_number">Nomor HBL</label><input id="hbl_number" name="hbl_number" maxlength="60" value="{{ old('hbl_number',$job->hbl_number) }}" placeholder="House Bill of Lading"></div>
...
<div class="field"><label for="shipment_reference">Referensi pengiriman</label><input id="shipment_reference" name="shipment_reference" maxlength="100" value="{{ old('shipment_reference',$job->shipment_reference) }}" placeholder="Referensi lain / nomor internal"></div>
<div class="field"><label for="booking_reference">Nomor booking</label><input id="booking_reference" name="booking_reference" maxlength="60" value="{{ old('booking_reference',$job->booking_reference) }}"></div>
</div>
<div class="form-section-heading"><h2>Muatan</h2><p>Ringkasan barang yang diangkut.</p></div>
<div class="form-grid">
...
<div class="field"><label for="package_count">Jumlah paket</label><input id="package_count" name="package_count" type="number" min="0" max="999999" value="{{ old('package_count',$job->package_count) }}"></div>
...
<div class="field"><label for="gross_weight">Berat kotor</label><input id="gross_weight" name="gross_weight" inputmode="decimal" value="{{ old('gross_weight',$job->gross_weight) }}" placeholder="contoh: 1250.50"><small>Dalam satuan kilogram (kg).</small></div>
<div class="field"><label for="volume">Volume muatan</label><input id="volume" name="volume" inputmode="decimal" value="{{ old('volume',$job->volume) }}" placeholder="contoh: 15.75"><small>Dalam satuan cubic meter (cbm).</small></div>
</div>
